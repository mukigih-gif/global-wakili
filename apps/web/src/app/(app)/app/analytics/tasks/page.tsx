...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    api.get<{ data: any[] }>('/tasks/search?limit=50')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><CheckSquare className="h-6 w-6 icon-tasks" /> Task Completion Report</h1><p className="text-sm text-gray-500">Completion rates by team</p></div>
      </div>
      {loading ? <div className="text-center py-8 text-sm text-gray-400">Loading…</div> :
       !data.length ? (
        <div className="card p-8 text-center">
          <p className="text-sm text-gray-600">No task completion data yet. Use the system and data will appear here.</p>
