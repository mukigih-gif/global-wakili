...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    api.get<{ data: any[] }>('/billing/invoices?limit=50')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><DollarSign className="h-6 w-6 icon-finance" /> Billing Summary Report</h1><p className="text-sm text-gray-500">Invoiced and paid amounts</p></div>
      </div>
      {loading ? <div className="text-center py-8 text-sm text-gray-400">Loading…</div> :
       !data.length ? (
        <div className="card p-8 text-center">
          <p className="text-sm text-gray-600">No billing summary data yet. Use the system and data will appear here.</p>
