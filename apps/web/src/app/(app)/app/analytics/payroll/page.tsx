...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    api.get<{ data: any[] }>('/hr/payroll?limit=50')
  ...
  return (
    <div className="space-y-5">
      <div className="flex items-center gap-3">
        ...
        <div><h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2"><UserCheck className="h-6 w-6 icon-hr" /> Payroll Report Report</h1><p className="text-sm text-gray-500">PAYE SHIF NSSF summary</p></div>
      </div>
      {loading ? <div className="text-center py-8 text-sm text-gray-400">Loading…</div> :
       !data.length ? (
        <div className="card p-8 text-center">
          <p className="text-sm text-gray-600">No payroll report data yet. Use the system and data will appear here.</p>
